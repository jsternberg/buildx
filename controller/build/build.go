...
func RunBuild(ctx context.Context, dockerCli command.Cli, in *Options, inStream io.Reader, progress progress.Writer, h build.Handler, generateResult bool) (*client.SolveResponse, *build.Inputs, error) {
	...
	resp, err := buildTargets(ctx, dockerCli, nodes, buildOptions, progress, h)
...
// this function returns it in addition to the error (i.e. it does "return nil, res, err"). The caller can
// inspect the result and debug the cause of that error.
func buildTargets(ctx context.Context, dockerCli command.Cli, nodes []builder.Node, opts map[string]build.Options, progress progress.Writer, h build.Handler) (*client.SolveResponse, error) {
