...
type LaunchConfig struct {
	Invoke *build.InvokeConfig `json:"invoke,omitempty"`
}
...
func (d *Adapter) start(c Context, cfg *build.InvokeConfig) {
	if cfg != nil {
		d.cfg = *cfg
	}
	c.Go(d.launch)
...
func (t *thread) Evaluate(ctx Context, c gateway.Client, res *gateway.Result) error {
	...
	if reason, desc := t.needsDebug(err); reason != "" {
		rCtx := build.NewResultHandle(ctx, c, res, err)
